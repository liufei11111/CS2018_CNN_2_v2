...
class ThreeLayerConvNet(object):
    ...
    def __init__(self, input_dim=(3, 32, 32), num_filters=32, filter_size=7,
        ...
        h_params = int((H-pool_param['pool_height'])/pool_param['stride']+1)
        w_params = int((W-pool_param['pool_width'])/pool_param['stride']+1)

        # Set weights and biases dimension
        weigths_dim = [(num_filters, C, filter_size, filter_size),
                       (h_params*w_params*num_filters, hidden_dim),
                       (hidden_dim, num_classes)]
        biases_dim = [num_filters, hidden_dim, num_classes]

        # Initialize weights and biases
        for i in range(1,4):
            self.params['W{:d}'.format(i)] = np.random.normal(loc=0.0, scale=weight_scale,
                                                     size=weigths_dim[i-1])
            self.params['b{:d}'.format(i)] = np.zeros(biases_dim[i-1])
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
